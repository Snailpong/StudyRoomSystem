...
public class FindPwd extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View arg0) {
        ...
        if(email.isEmpty()||email.equals("")||!email.contains("@")||!email.contains(".")){
            Toast.makeText(FindPwd.this, "이메일을 입력해 주세요.", Toast.LENGTH_SHORT).show();
        }
            ...
            Intent in = new Intent(FindPwd.this, EmailLink.class);
            in.putExtra("auth_string", auth_string);
            in.putExtra("auth_email", et.getText().toString());
            startActivity(in);
        }
        else{
            Toast.makeText(FindPwd.this, "부산대학교 이메일을 입력해 주세요. (ex) ABC.pusan.ac.kr)", Toast.LENGTH_LONG).show();
        }
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    Toast.makeText(FindPwd.this, "전송 실패", Toast.LENGTH_LONG).show();
                }
                dialog.dismiss();
            }
